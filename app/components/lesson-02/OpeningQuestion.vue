<template>
  <section id="opening" class="min-h-screen relative flex flex-col items-center justify-center py-24 px-6 sm:px-12 bg-surface overflow-hidden">
    ...
    <div class="max-w-5xl w-full mx-auto text-center z-10" ref="containerRef">
      ...
      <h3 class="text-3xl sm:text-4xl lg:text-5xl font-bold leading-[1.3] mb-20 text-maintext text-balance">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-16 relative">
        <button v-for="option in data.openingQuestion.options" :key="option.id"
          ...
          <div class="text-2xl sm:text-3xl font-black transition-colors relative z-10"
      ...
      <div class="min-h-[200px] w-full flex items-start justify-center">
        <div v-if="showFeedback"
             ref="feedbackRef"
             class="w-full bg-gradient-to-br from-primary to-[#6350d4] text-white p-10 md:p-12 rounded-[2.5rem] shadow-2xl shadow-primary/30 flex flex-col md:flex-row items-center md:items-start gap-8 text-right opacity-0 translate-y-12 border border-white/20">
          ...
          <p class="text-xl md:text-3xl font-medium leading-[1.6] text-balance">
