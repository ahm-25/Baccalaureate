<template>
  <div class="flex flex-col gap-6 mt-6 font-arabic text-right">
    <p class="text-slate-500 text-sm">اسحب العناصر لترتيبها بالشكل الصحيح من الأعلى إلى الأسفل.</p>
    ...
    <div class="flex flex-col gap-3">
      <div 
        ...
        :class="[
          ...
          dragOverIndex === index ? 'border-primary border-t-8' : 'border-b-[6px] hover:border-primary/50 cursor-grab active:cursor-grabbing active:translate-y-0.5 active:border-b-2'
        ]"
      >
        ...
        <div class="text-slate-400 opacity-40 cursor-grab" v-if="!isAnswered">
    ...
    <button 
      v-if="!isAnswered"
      @click="checkAnswers"
      class="mt-4 px-8 py-4 rounded-xl font-bold transition-all text-white w-full md:w-auto self-center bg-primary hover:bg-primary/95 hover:shadow-lg hover:shadow-primary/30 transform hover:-translate-y-0.5"
    >
      تحقق من الترتيب
    </button>
